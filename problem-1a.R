...
MIPModel() %>%
  ...
  set_objective(0.35*x[1] + 0.25*x[2], "max") %>%
  
  #================================================
  #constrains
  #space per jar is 50cm2 and the max space is 5000cm2
  #own brand"s sales are always half of others brand's sales
  add_constraint(50*x[1] + 50*x[2] <= 5000) %>% #shelf space
  add_constraint(x[1] <= x[2]/2) %>% #sales
  set_bounds(x[i], lb=0, i=1:2) %>% #set bounds for x1,x2
...
objective <- c(
        0.35,  #profit per jar for own brand (x1)
